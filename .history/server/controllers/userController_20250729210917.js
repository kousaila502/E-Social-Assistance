...
const getUserStats = async (req, res) => {
  ...
  try {
    ...
    // =====================================================
    // AGGREGATE USER'S REQUESTS
    // =====================================================
    console.log('📋 Aggregating user requests...');
      ...
      { $match: { applicant: user._id } },
      {
        $group: {
          ...
          pendingRequests: {
            $sum: {
              $cond: [
                { $in: ['$status', ['submitted', 'under_review', 'pending_docs']] },
                1,
                0
              ]
            }
    ...
    const userRequestStats = requestStats[0] || {
      totalRequests: 0,
      totalRequested: 0,
      totalApproved: 0,
      approvedRequests: 0,
    ...
    console.log('📋 Request stats:', userRequestStats);

    // Calculate average processing time
    const validProcessingTimes = userRequestStats.completedRequests.filter(req => req !== null);
    const averageProcessingTime = validProcessingTimes.length > 0
      ? Math.round(validProcessingTimes.reduce((sum, req) => sum + req.processingTime, 0) / validProcessingTimes.length)
      : 0;

    // =====================================================
    // AGGREGATE USER'S PAYMENTS
    // =====================================================
    console.log('💰 Aggregating user payments...');
    ...
    try {
      ...
      if (paymentStats.length > 0) {
        totalAmountReceived = paymentStats[0].totalAmountReceived || 0;

        // Get recent payments for activity timeline
        paymentActivities = paymentStats[0].payments
          .sort((a, b) => new Date(b.processedAt || b.updatedAt) - new Date(a.processedAt || a.updatedAt))
          .slice(0, 10)
          .map(payment => ({
            id: `payment-${payment._id}`,
            type: 'payment_received',
            description: `Received payment of ${payment.amount.toLocaleString()} DA`,
            timestamp: payment.processedAt || payment.updatedAt,
            metadata: {
      ...
      console.log('💰 Payment stats:', { totalAmountReceived, paymentsCount: paymentActivities.length });
    } catch (paymentError) {
      console.warn('💰 Payment aggregation failed (Payment model might not exist):', paymentError.message);
    ...
    // =====================================================
    console.log('📋 Building activity timeline...');

    // Get user's recent requests for activity timeline
    const recentRequests = await Demande.find({ applicant: user._id })
      .sort({ updatedAt: -1 })
      .limit(10)
      .lean();
    ...
    recentRequests.forEach(request => {
      // Request submitted activity
      requestActivities.push({
        id: `request-submitted-${request._id}`,
      ...
      // Request status change activities
      if (['approved', 'paid'].includes(request.status)) {
        requestActivities.push({
          id: `request-approved-${request._id}`,
          type: 'request_approved',
          description: `Request approved: ${request.title}`,
        ...
        requestActivities.push({
          id: `request-rejected-${request._id}`,
          type: 'request_approved', // Reuse same type for UI consistency
          description: `Request rejected: ${request.title}`,
          timestamp: request.updatedAt,
          metadata: {
            requestTitle: request.title,
            status: request.status
    ...
    // Add document activities from user documents
    const documentActivities = [];
    if (user.documents) {
      Object.entries(user.documents).forEach(([docType, docData]) => {
        if (docData.verifiedAt) {
          documentActivities.push({
            id: `document-${docType}`,
            type: 'document_uploaded',
            description: `Uploaded ${docType.replace(/([A-Z])/g, ' $1').toLowerCase()}`,
            timestamp: docData.verifiedAt,
            metadata: {
    ...
    const allActivities = [
      ...
      .slice(0, 15); // Limit to last 15 activities
    ...
    const responseData = {
      statistics: {
        totalRequests: userRequestStats.totalRequests,
        approvedRequests: userRequestStats.approvedRequests,
        rejectedRequests: userRequestStats.rejectedRequests,
        pendingRequests: userRequestStats.pendingRequests,
    ...
    console.log('📊 Final response stats:', responseData.statistics);
