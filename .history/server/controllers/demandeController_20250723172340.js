...
const createDemande = async (req, res) => {
  ...
  if (!title || !description || !requestedAmount) {
    throw new CustomError.BadRequestError(
      'Please provide title, description, and requested amount'
    );
  }
  ...
  try {
    // Get user (basic check only)
    const user = await User.findById(req.user.userId);
    if (!user) {
      throw new CustomError.NotFoundError('User not found');
    }

    // Handle program reference
    let programReference = null;

    if (program && program.type && program.id) {
      // If program ID is a string identifier, we need to find the actual document
      if (program.type === 'Content') {
        ...
        if (!contentDoc) {
          ...
          // Create a default content document for testing
          contentDoc = await Content.create({
            ...
            contentType: 'programme', // Required field with correct enum value
            status: 'active',
            visibility: 'public',
            level: 0,
        ...
        let announcementDoc = await Announcement.findOne({
          $or: [
        ...
        if (announcementDoc) {
          ...
          throw new CustomError.NotFoundError(`Announcement '${program.id}' not found`);
        }
      }
    }
    ...
    if (!programReference) {
      ...
      if (!defaultContent) {
        ...
        defaultContent = await Content.create({
          ...
          contentType: 'programme', // Required field
          status: 'active',
          visibility: 'public',
          level: 0,
          createdBy: req.user.userId,
    ...
    const requestNumber = `REQ-${Date.now()}-${String(requestCount + 1).padStart(4, '0')}`;

    // Calculate eligibility score (simplified)
    const eligibilityScore = 75; // Default score for testing
