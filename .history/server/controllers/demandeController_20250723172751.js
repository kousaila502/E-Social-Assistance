...
const createDemande = async (req, res) => {
  console.log('=== CREATE DEMANDE REQUEST ===');
  ...
  try {
    ...
    if (!requestedAmount) {
      validationErrors.push('Requested amount is required');
    } else if (typeof requestedAmount !== 'number' || requestedAmount <= 0) {
      validationErrors.push('Requested amount must be a positive number');
    } else if (requestedAmount > 1000000) {
      validationErrors.push('Requested amount cannot exceed 1,000,000 DA');
    }
    ...
    const validUrgencyLevels = ['routine', 'important', 'urgent', 'critical'];
    if (urgencyLevel && !validUrgencyLevels.includes(urgencyLevel)) {
      validationErrors.push(`Urgency level must be one of: ${validUrgencyLevels.join(', ')}`);
    }
    ...
    if (tags && Array.isArray(tags)) {
      tags.forEach((tag, index) => {
        if (typeof tag !== 'string' || tag.trim().length === 0) {
          validationErrors.push(`Tag at position ${index + 1} must be a non-empty string`);
        } else if (tag.length > 50) {
          validationErrors.push(`Tag "${tag}" is too long (maximum 50 characters)`);
        }
      });
    }
    ...
    if (!user) {
      throw new CustomError.NotFoundError('User account not found. Please log in again.');
    }

    if (user.accountStatus !== 'active') {
      throw new CustomError.UnauthorizedError(
        `Cannot create requests with account status: ${user.accountStatus}. Please contact support.`
      );
    }

    // Check if user has reached request limit (optional business rule)
    const activeRequestsCount = await Demande.countDocuments({
      applicant: req.user.userId,
      status: { $in: ['draft', 'submitted', 'under_review', 'pending_docs'] },
      isDeleted: false
    });

    const maxActiveRequests = 5; // Business rule: max 5 active requests per user
    if (activeRequestsCount >= maxActiveRequests) {
      throw new CustomError.BadRequestError(
        `You have reached the maximum of ${maxActiveRequests} active requests. Please wait for existing requests to be processed.`
      );
    }

    // Handle program reference with enhanced error handling
    let programReference = null;

    if (program && program.type && program.id) {
      if (program.type === 'Content') {
        // Try to find Content by name or create a default one
        let contentDoc = await Content.findOne({ name: program.id });
        if (!contentDoc) {
          try {
            // Generate a unique content number
            const contentCount = await Content.countDocuments();
            const contentNumber = `CNT-${Date.now()}-${String(contentCount + 1).padStart(4, '0')}`;

            // Create a default content document
            contentDoc = await Content.create({
              contentNumber,
              name: program.id,
              title: `${category.replace('_', ' ').toUpperCase()} Program`,
              description: `Default program for ${category.replace('_', ' ')} assistance requests`,
            ...
            console.log('Created default content document:', contentDoc._id);
          } catch (contentError) {
            console.error('Error creating content document:', contentError);
            throw new CustomError.BadRequestError(
              'Failed to create program reference. Please try again or contact support.'
            );
          }
        }
        programReference = {
        ...
        // Try to find Announcement by title or ID
        let announcementDoc = null;
        
        try {
          announcementDoc = await Announcement.findOne({
            $or: [
              { _id: mongoose.Types.ObjectId.isValid(program.id) ? program.id : null },
              { title: { $regex: program.id, $options: 'i' } }
            ],
            status: 'active' // Only active announcements
          });
        } catch (findError) {
          console.error('Error finding announcement:', findError);
          throw new CustomError.BadRequestError('Invalid announcement reference');
        }

        if (announcementDoc) {
          // Check if announcement is still accepting applications
          if (announcementDoc.deadline && new Date() > announcementDoc.deadline) {
            throw new CustomError.BadRequestError(
              `The announcement "${announcementDoc.title}" is no longer accepting applications (deadline passed)`
            );
          }
          ...
          throw new CustomError.NotFoundError(
            `Announcement '${program.id}' not found or is no longer active`
          );
        }
      } else {
        throw new CustomError.BadRequestError(
          'Program type must be either "Content" or "Announcement"'
        );
      }
    }

    // If no valid program reference, create a default Content
    if (!programReference) {
      let defaultContent = await Content.findOne({ name: 'default_assistance_program' });
      if (!defaultContent) {
        try {
          // Generate a unique content number
          const contentCount = await Content.countDocuments();
          ...
          defaultContent = await Content.create({
            contentNumber,
            name: 'default_assistance_program',
            title: 'General Assistance Program',
            description: 'Default program for general assistance requests',
            contentType: 'programme',
            status: 'active',
            visibility: 'public',
            level: 0,
          ...
          console.log('Created default assistance content:', defaultContent._id);
        } catch (defaultContentError) {
          console.error('Error creating default content:', defaultContentError);
          throw new CustomError.BadRequestError(
            'Failed to create default program. Please contact support.'
          );
        }
      }
      programReference = {
        type: 'Content',
        id: defaultContent._id
      };
    }

    // Generate request number with error handling
    let requestNumber;
    try {
      const requestCount = await Demande.countDocuments();
      requestNumber = `REQ-${Date.now()}-${String(requestCount + 1).padStart(4, '0')}`;
    } catch (countError) {
      console.error('Error generating request number:', countError);
      requestNumber = `REQ-${Date.now()}-${Math.floor(Math.random() * 9999).toString().padStart(4, '0')}`;
    }
    ...
    // Create the request with comprehensive error handling
    let demande;
    try {
      demande = await Demande.create({
        requestNumber,
        title: title.trim(),
      ...
      console.log('Demande created successfully:', demande._id);
    } catch (createError) {
      console.error('Error creating demande:', createError);
      
      // Handle specific creation errors
      if (createError.name === 'ValidationError') {
        const validationMessages = Object.values(createError.errors).map(err => err.message);
        throw new CustomError.BadRequestError(
          `Request creation failed: ${validationMessages.join('; ')}`
        );
      } else if (createError.code === 11000) {
        throw new CustomError.BadRequestError(
          'A request with this information already exists. Please check your request details.'
        );
      } else {
        throw new CustomError.BadRequestError(
          'Failed to create request. Please check your information and try again.'
        );
      }
    }

    // Populate the created demande with error handling
    let populatedDemande;
    try {
      populatedDemande = await Demande.findById(demande._id)
        .populate('applicant', 'firstName lastName email eligibility')
        .populate('createdBy', 'firstName lastName email')
        .populate('program.id', 'name title description');

      if (!populatedDemande) {
        throw new CustomError.NotFoundError('Request was created but could not be retrieved');
      }
    } catch (populateError) {
      console.error('Error populating demande:', populateError);
      // Return the basic demande if population fails
      populatedDemande = demande;
    }
    ...
    if (error.statusCode) {
      // This is already a custom error, re-throw it
      throw error;
    }
    ...
    if (error.name === 'CastError') {
      throw new CustomError.BadRequestError(
        `Invalid ${error.path}: ${error.value}. Please provide a valid ${error.path}.`
      );
    }
    ...
    if (error.code === 11000) {
      ...
      throw new CustomError.BadRequestError(
        `Duplicate value for ${field}: "${value}". This value is already in use.`
      );
    }
    ...
    console.error('Unexpected error in createDemande:', error);
    throw new CustomError.BadRequestError(
      'An unexpected error occurred while creating your request. Please try again or contact support.'
    );
  }
};
