...
// Instance method to cancel allocation
BudgetPoolSchema.methods.cancelAllocation = function(demandeId, userId, reason) {
  // Find the allocation
  const allocation = this.allocations.find(
    alloc => alloc.demande.toString() === demandeId.toString()
  );
  ...
  if (allocation) {
    ...
    allocation.notes = `Cancelled: ${reason}`;
    
    // Release reserved funds
    this.reservedAmount = Math.max(0, this.reservedAmount - allocation.amount);
  }
