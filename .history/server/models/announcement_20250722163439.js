...
// Normalize targetAudience on save
AnnouncementSchema.pre('save', function (next) {
  // Handle legacy string format
  if (typeof this.targetAudience === 'string') {
    const legacyValue = this.targetAudience;
    this.targetAudience = {
      userTypes: ['all'],
      categories: [legacyValue === 'all' ? 'all' : legacyValue],
      eligibilityCategories: [],
      locations: []
    };
  }

  // Ensure targetAudience is always an object with required properties
  if (!this.targetAudience || typeof this.targetAudience !== 'object') {
    this.targetAudience = {
      userTypes: ['all'],
      categories: ['all'],
      eligibilityCategories: [],
  ...
  this.targetAudience.userTypes = this.targetAudience.userTypes || ['all'];
  this.targetAudience.categories = this.targetAudience.categories || ['all'];
  this.targetAudience.eligibilityCategories = this.targetAudience.eligibilityCategories || [];
  this.targetAudience.locations = this.targetAudience.locations || [];

  next();
});

// Handle documents loaded from database
AnnouncementSchema.post('init', function (doc) {
  // Normalize targetAudience when loading from database
  if (typeof doc.targetAudience === 'string') {
    const legacyValue = doc.targetAudience;
    doc.targetAudience = {
      userTypes: ['all'],
      categories: [legacyValue === 'all' ? 'all' : legacyValue],
      eligibilityCategories: [],
  ...
  // Ensure required arrays exist
  doc.targetAudience.userTypes = doc.targetAudience.userTypes || ['all'];
  doc.targetAudience.categories = doc.targetAudience.categories || ['all'];
  doc.targetAudience.eligibilityCategories = doc.targetAudience.eligibilityCategories || [];
  doc.targetAudience.locations = doc.targetAudience.locations || [];
});
...
AnnouncementSchema.pre('findOneAndUpdate', function (next) {
  ...
  if (update.status || update.expiresAt) {
    const status = update.status || this._conditions.status;
    const expiresAt = update.expiresAt || this._conditions.expiresAt;
