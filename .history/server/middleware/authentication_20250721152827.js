  ...
  try {
    ...
    if (accessToken) {
      try {
        ...
        const user = await User.findById(payload.user.userId).select('+isActive +accountStatus');
        if (!user) {
    ...
    // Verify user still exists and is active
    const user = await User.findById(refreshPayload.user.userId).select('+isActive +accountStatus');
    if (!user) {
      throw new CustomError.UnauthenticatedError('User account not found');
    }
...
const requireActiveAccount = async (req, res, next) => {
  try {
    ...
    if (user.accountStatus !== 'active') {
      throw new CustomError.UnauthorizedError(`Account is ${user.accountStatus}. Active account required.`);
    }

    if (!user.isEmailVerified) {
      throw new CustomError.UnauthorizedError('Email verification required to proceed.');
    }
