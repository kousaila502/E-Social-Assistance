...
// Set cookies with access and refresh tokens - FIXED FOR CROSS-ORIGIN
const attachCookiesToResponse = ({ res, user, refreshToken, rememberMe = false }) => {
  ...
  const cookieOptions = {
    httpOnly: true,
    secure: true, // Always true for cross-origin (HTTPS required)
    signed: true,
...
// Clear authentication cookies - FIXED FOR CROSS-ORIGIN
const clearTokenCookies = (res) => {
  const cookieOptions = {
    httpOnly: true,
    secure: true, // Always true for cross-origin
    signed: true,
    sameSite: 'none', // CRITICAL: Must match the setting used when creating cookies
    path: '/'
  };
