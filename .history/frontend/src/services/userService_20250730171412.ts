...
export interface UserStats {
  ...
  pendingRequests?: number; // Add this since backend returns it
...
const userService = {
  ...
  getUserStats: async (id: string): Promise<{
    ...
    try {
      const response: {
        ...
        activity: UserActivity[];
        user: any;
      } = await api.get(API_CONFIG.ENDPOINTS.USERS.GET_STATS(id));
      return response;
    } catch (error: any) {
      throw error;
    }
  ...
  export: async (format: 'csv' | 'json' = 'csv', filters?: UserFilters): Promise<Blob> => {
    try {
      ...
      // FIX: Use axios directly to bypass the interceptor for blob responses
      const response = await axios.get(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.USERS.GET_ALL}/export?${queryParams.toString()}`, {
        responseType: 'blob',
        withCredentials: true
      });
      return response.data; // Return the actual blob data
    } catch (error: any) {
      throw error;
    }
  },
  ...
  downloadDocument: async (userId: string, documentType: string): Promise<Blob> => {
    try {
      // FIX: Use axios directly to bypass the interceptor for blob responses
      const response = await axios.get(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.USERS.DOWNLOAD_DOCUMENT(userId, documentType)}`, {
        responseType: 'blob',
        withCredentials: true
      });
      return response.data; // Return the actual blob data
  ...
  getUserDocuments: async (userId: string): Promise<{ documents: any }> => {
    try {
      // The api interceptor returns response.data, which matches our return type
      const response: { documents: any } = await api.get(API_CONFIG.ENDPOINTS.USERS.GET_DOCUMENTS(userId));
      return response; // This is already response.data due to interceptor
    } catch (error: any) {
      throw error;
    }
  },
  ...
  deleteDocument: async (userId: string, documentType: string): Promise<{ message: string }> => {
    try {
      // The api interceptor returns response.data, which matches our return type
      const response: { message: string } = await api.delete(API_CONFIG.ENDPOINTS.USERS.DELETE_DOCUMENT(userId, documentType));
      return response; // This is already response.data due to interceptor
    } catch (error: any) {
      throw error;
    }
