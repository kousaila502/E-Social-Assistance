...
export interface UserDetails extends User {
  ...
  // Remove the eligibility redefinition - it inherits from User
}
...
const userService = {
  ...
  getRecentRegistrations: async (days: number = 7): Promise<UserDetails[]> => {
    try {
  ...
  getStaffMembers: async (): Promise<UserDetails[]> => {
    try {
  ...
  canEdit: (user: UserDetails, currentUserRole: string): boolean => {
  ...
  canDelete: (user: UserDetails, currentUserRole: string): boolean => {
    if (currentUserRole === 'admin' && user.role !== 'admin') return true;
    return false;
  },

  // Helper function to get user statistics summary
  getUserStatsSummary: (user: UserDetails): string => {
