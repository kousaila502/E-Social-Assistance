...
const budgetService = {
  ...
  getBudgetPool: async (id: string): Promise<BudgetPoolResponse> => {
    try {
      const response: BudgetPoolResponse = await api.get(`${API_CONFIG.ENDPOINTS.BUDGET_POOLS}/${id}`);
      return response;
    } catch (error: any) {
      throw error;
    }
  ...
  getDashboardStats: async (): Promise<BudgetDashboardStats> => {
    try {
      const response: BudgetDashboardStats = await api.get(`${API_CONFIG.ENDPOINTS.BUDGET_POOLS}/dashboard-stats`);
      return response;
    } catch (error: any) {
      throw error;
    }
