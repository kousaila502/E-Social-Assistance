...
const announcementService = {
  ...
  // Get my applications (for regular users)
  getMyApplications: async (params?: {
    ...
    try {
      ...
      const url = `${API_CONFIG.ENDPOINTS.ANNOUNCEMENTS.GET_ALL}/my-applications?${queryParams.toString()}`;
  ...
  // Get announcement participants (for staff)
  getParticipants: async (id: string, params?: {
    page?: number;
    limit?: number;
    status?: 'pending' | 'accepted' | 'rejected' | 'waitlisted';
  }): Promise<{
    ...
    try {
      ...
      const url = `${API_CONFIG.ENDPOINTS.ANNOUNCEMENTS.GET_BY_ID(id)}/participants?${queryParams.toString()}`;
      const response = await api.get(url);
      return response;
    } catch (error: any) {
      throw error;
  ...
  // Check if user can apply to announcement
  canApply: async (id: string): Promise<{
    canApply: boolean;
    reason?: string;
    hasApplied?: boolean;
    ...
    try {
      const response = await api.get(`${API_CONFIG.ENDPOINTS.ANNOUNCEMENTS.GET_BY_ID(id)}/can-apply`);
      return response;
    } catch (error: any) {
      throw error;
    }
  ...
  formatTargetAudience: (audience: string): string => {
