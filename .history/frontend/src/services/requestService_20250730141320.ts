...
const requestService = {
  ...
  create: async (requestData: CreateDemandeData): Promise<MessageResponse> => {
    try {
      ...
      // Let the ErrorHandler format the error for components
      throw error; // Don't transform here, let components handle it
    }
  },
  ...
  updateStatus: async (id: string, status: RequestStatus): Promise<MessageResponse> => {
  try {
    const response: MessageResponse = await api.patch(
      `${API_CONFIG.ENDPOINTS.DEMANDES.UPDATE(id)}/status`, // or whatever your endpoint is
      { status }
    );
    return response;
  } catch (error: any) {
