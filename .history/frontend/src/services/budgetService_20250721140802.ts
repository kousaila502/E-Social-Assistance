...
const budgetService = {
  ...
  getBudgetPool: async (id: string): Promise<BudgetPoolResponse> => {
    try {
      const response: BudgetPoolResponse = await api.get(`${API_CONFIG.ENDPOINTS.BUDGET_POOLS.GET_BY_ID(id)}`);
      return response;
    } catch (error: any) {
      throw error;
    }
  ...
  createBudgetPool: async (data: CreateBudgetPoolData): Promise<{ message: string; budgetPool: BudgetPool }> => {
    try {
      const response: { message: string; budgetPool: BudgetPool } = await api.post(API_CONFIG.ENDPOINTS.BUDGET_POOLS.CREATE, data);
      return response;
    } catch (error: any) {
      throw error;
    }
  ...
  updateBudgetPool: async (id: string, data: UpdateBudgetPoolData): Promise<{ message: string; budgetPool: BudgetPool }> => {
    try {
      const response: { message: string; budgetPool: BudgetPool } = await api.patch(`${API_CONFIG.ENDPOINTS.BUDGET_POOLS.UPDATE(id)}`, data);
      return response;
    } catch (error: any) {
      throw error;
    }
  ...
  deleteBudgetPool: async (id: string): Promise<{ message: string }> => {
    try {
      const response: { message: string } = await api.delete(`${API_CONFIG.ENDPOINTS.BUDGET_POOLS.DELETE(id)}`);
      return response;
    } catch (error: any) {
      throw error;
    }
  ...
  allocateFunds: async (poolId: string, data: AllocateFundsData): Promise<{ message: string }> => {
    try {
      const response: { message: string } = await api.post(`${API_CONFIG.ENDPOINTS.BUDGET_POOLS.ALLOCATE(poolId)}`, data);
      return response;
    } catch (error: any) {
      throw error;
    }
  ...
  transferFunds: async (poolId: string, data: TransferFundsData): Promise<{ message: string }> => {
    try {
      const response: { message: string } = await api.post(`${API_CONFIG.ENDPOINTS.BUDGET_POOLS.TRANSFER(poolId)}`, data);
      return response;
    } catch (error: any) {
      throw error;
    }
  ...
  getDashboardStats: async (): Promise<BudgetDashboardStats> => {
    try {
      const response: BudgetDashboardStats = await api.get(`${API_CONFIG.ENDPOINTS.BUDGET_POOLS.DASHBOARD_STATS}`);
      return response;
    } catch (error: any) {
      throw error;
    }
  ...
  getPoolAnalytics: async (
    ...
    try {
      const response: BudgetPoolAnalytics = await api.get(
        `${API_CONFIG.ENDPOINTS.BUDGET_POOLS.ANALYTICS(poolId)}?period=${period}`
      );
      return response;
    } catch (error: any) {
      throw error;
  ...
  activatePool: async (poolId: string): Promise<{ message: string }> => {
    try {
      const response: { message: string } = await api.patch(`${API_CONFIG.ENDPOINTS.BUDGET_POOLS.UPDATE(poolId)}`, {
        status: 'active'
      });
      return response;
    } catch (error: any) {
