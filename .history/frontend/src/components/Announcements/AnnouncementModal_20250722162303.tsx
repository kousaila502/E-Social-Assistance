...
const CreateAnnouncementModal: React.FC<CreateAnnouncementModalProps> = ({
    ...
    useEffect(() => {
        if (editingAnnouncement) {
            ...
            let targetAudienceValue: 'all' | 'students' | 'families' | 'elderly' | 'disabled' | 'unemployed' | 'specific' = 'all'; // default fallback with proper type
            ...
            if (typeof editingAnnouncement.targetAudience === 'string') {
                ...
                // If it's a complex object, extract the best matching string value
                const targetObj = editingAnnouncement.targetAudience;

                // Priority order: userTypes -> eligibilityCategories -> categories
                if (targetObj.userTypes && targetObj.userTypes.length > 0) {
                    const userType = targetObj.userTypes[0];
                    // Map server userTypes to form values
                    ...
                    // Map eligibility categories to form values
    ...
    const SelectField: React.FC<{
        ...
        required?: boolean;
        errors: Record<string, string>;
    }> = ({ label, name, value, onChange, options, required, errors }) => (
        <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
                {label}
                {required && <span className="text-red-500 ml-1">*</span>}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                <InputField
                                    ...
                                    required
                                    maxLength={200}
                                    Property 'errors' is missing in type '{ label: string; name: string; value: string; onChange: (value: string | number) => void; placeholder: string; required: true; maxLength: number; }' but required in type '{ label: string; name: string; type?: string | undefined; value: string | number; onChange: (value: string | number) => void; placeholder?: string | undefined; required?: boolean | undefined; rows?: number | undefined; maxLength?: number | undefined; min?: number | undefined; errors: Record<...>; }'.ts(2741)
AnnouncementModal.tsx(293, 9): 'errors' is declared here.
                                />
