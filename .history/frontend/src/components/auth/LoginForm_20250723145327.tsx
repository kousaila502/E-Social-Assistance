...
const LoginForm: React.FC<LoginFormProps> = ({ onSuccess }) => {
  ...
  useEffect(() => {
    ...
    // Display success message if redirected from registration, then clear it
    if (location.state?.message) {
      ...
      // Clear the state to prevent re-showing on reload
      window.history.replaceState({}, document.title);
    }
  }, [clearError]); // Remove location dependency to prevent re-triggering
  ...
  const getErrorMessage = (errorCode: string, originalMessage: string) => {
    ...
    // Return specific error message if we have one, otherwise use original message
  ...
  const onSubmit = async (data: LoginCredentials) => {
    ...
    try {
      ...
      // Handle the new error structure from apiConfig
