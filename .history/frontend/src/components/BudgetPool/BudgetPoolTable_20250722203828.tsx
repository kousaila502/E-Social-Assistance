...
import { BudgetPool, BudgetPoolFilters } from '../../config/apiConfig';
...
interface BudgetPoolTableProps {
    ...
    filters?: Partial<BudgetPoolFilters>;
...
const BudgetPoolTable: React.FC<BudgetPoolTableProps> = ({
    ...
    const fetchBudgetPools = async (page: number = 1) => {
        ...
        try {
            const params: BudgetPoolFilters = {
                page,
                limit: itemsPerPage,
                sortBy: sortConfig.key as BudgetPoolFilters['sortBy'], // Add type assertion
    ...
    const isLowBalance = (budgetPool: BudgetPool) => {
        ...
        return utilization >= (100 - budgetPool.alertThresholds.lowBalanceWarning);
    };

    const isCriticalBalance = (budgetPool: BudgetPool) => {
        if (!budgetPool.alertThresholds) return false;
        const utilization = calculateUtilization(budgetPool);
        return utilization >= (100 - budgetPool.alertThresholds.criticalBalanceAlert);
    };
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center">
                                                        <div className="w-16 bg-gray-200 rounded-full h-2 mr-2">
                                                            <div
                                                                className={`h-2 rounded-full transition-all duration-300 ${criticalBalance ? 'bg-red-500' :
                                                        ...
                                                        <span className={`text-sm font-medium ${criticalBalance ? 'text-red-600' :
