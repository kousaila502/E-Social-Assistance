...
const UserTable: React.FC<UserTableProps> = ({
  ...
  try {
    ...
    console.log('🔍 Fetching users with params:', apiParams); // Debug log
    
    const response = await userService.getAll(apiParams);
    
    console.log('📊 API returned:', response.users.length, 'users'); // Debug log
    console.log('📋 First few users status:', response.users.slice(0, 3).map(u => ({name: u.name, status: u.accountStatus}))); // Debug log
