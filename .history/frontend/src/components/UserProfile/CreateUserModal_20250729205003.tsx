...
const CreateUserModal: React.FC<CreateUserModalProps> = ({
    ...
    const validateForm = (): boolean => {
        ...
        if (formData.password) {
            ...
            if (formData.password.length < minLength) {
                passwordErrors.push(`at least ${minLength} characters`);
            }
            if (!hasUpperCase) passwordErrors.push('at least one uppercase letter');
            if (!hasLowerCase) passwordErrors.push('at least one lowercase letter');
            if (!hasNumbers) passwordErrors.push('at least one number');
            if (!hasSpecialChar) passwordErrors.push('at least one special character');
        ...
        if (formData.phoneNumber) {
            ...
            if (!localFormat && !internationalFormat) {
                if (cleaned.startsWith('0') && cleaned.length !== 10) {
                    newErrors.phoneNumber = `Local phone number must be exactly 10 digits starting with 0 (you provided ${cleaned.length} digits)`;
                } else if (cleaned.startsWith('+213') && cleaned.length !== 13) {
                    newErrors.phoneNumber = `International phone number must be +213 followed by exactly 9 digits (you provided ${cleaned.length - 4} digits after +213)`;
                } else if (cleaned.startsWith('+') && !cleaned.startsWith('+213')) {
                    newErrors.phoneNumber = 'International phone number must start with +213 (Algeria country code)';
                } else {
                    newErrors.phoneNumber = 'Phone number must start with 0 for local format (0xxxxxxxxx) or +213 for international format (+213xxxxxxxxx)';
                }
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            console.log('Calling authService.register with:', formData); // ← Add this debug
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="0123456789 or +213123456789"
                                    />
