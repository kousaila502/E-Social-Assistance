  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      ...
      if (newStatus === 'cancelled') {
        const result = await requestService.cancel(request._id, { reason: 'Status changed by administrator' });
        console.log('🔍 Cancel result:', result); // Add this
      } else {
        const result = await requestService.update(request._id, { status: newStatus });
        console.log('🔍 Update result:', result); // Add this - This is KEY!
      }

      ErrorHandler.showSuccess('Status updated successfully!');

      // Add debugging for the refresh
      console.log('🔍 Fetching updated request...'); // Add this
      const updatedRequest = await requestService.getById(request._id);
      console.log('🔍 Updated request status:', updatedRequest.demande.status); // Add this
