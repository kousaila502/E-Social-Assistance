...
const StatusUpdater: React.FC<StatusUpdaterProps> = ({
  ...
  const handleAssign = async (data: AssignFormData) => {
    if (!canAssign) {
      ErrorHandler.showError('You do not have permission to assign requests.', 'Access Denied');
      return;
    }
    ...
    try {
      await requestService.assign(request._id, { assignedTo: data.assignedTo });
      ...
      ErrorHandler.showError(error, 'Assignment Failed');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      ...
      if (newStatus === 'cancelled') {
        ...
        // For other status changes, you might need specific endpoints
        ErrorHandler.showInfo('Status change functionality will be implemented with specific endpoints.');
        return;
      }
