  ...
  const fetchRequests = async (page: number = 1) => {
    ...
    try {
      ...
      if (response.pagination) {
        ...
        setTotalCount(response.pagination.totalDemandes || 0); // Changed from totalCount to totalDemandes
      } else {
  ...
  const handleSort = (key: keyof Demande | 'applicantName') => {
    ...
    const sortedRequests = [...requests].sort((a, b) => {
      ...
      if (key === 'applicantName') {
        aValue = typeof a.applicant === 'object' && a.applicant?.name ? a.applicant.name : '';
        bValue = typeof b.applicant === 'object' && b.applicant?.name ? b.applicant.name : '';
      } else {
        aValue = a[key];
        bValue = b[key];
      }
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">
                          {typeof request.applicant === 'object' && request.applicant?.name
                            ? request.applicant.name
                            : 'Unknown'}
                        </div>
                        <div className="text-sm text-gray-500">
                          {typeof request.applicant === 'object' && request.applicant?.email
