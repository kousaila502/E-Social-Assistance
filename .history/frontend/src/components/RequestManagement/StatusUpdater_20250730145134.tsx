...
const StatusUpdater: React.FC<StatusUpdaterProps> = ({
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      console.log('🔍 Updating status from:', request.status, 'to:', newStatus); // Add this
      console.log('🔍 Request ID:', request._id); // Add this

      if (newStatus === 'cancelled') {
        await requestService.cancel(request._id, { reason: 'Status changed by administrator' });
      } else {
        // Add debugging here
        console.log('🔍 Calling requestService.update with:', { status: newStatus }); // Add this
