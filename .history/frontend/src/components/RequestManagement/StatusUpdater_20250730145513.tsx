  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      ...
      if (newStatus === 'cancelled') {
        updateResult = await requestService.cancel(request._id, { reason: 'Status changed by administrator' });
      } else {
        updateResult = await requestService.update(request._id, { status: newStatus });
      }
      ...
      // Use the demande from the update result instead of fetching again
      if (updateResult.demande) {
        console.log('🔍 Using updated demande from response:', updateResult.demande.status);
        onStatusUpdate(updateResult.demande);
      } else {
        // Fallback: fetch fresh data only if update didn't return the demande
