...
const StatusUpdater: React.FC<StatusUpdaterProps> = ({
  ...
  const handleCancel = async (data: CancelFormData) => {
    setIsSubmitting(true);
    try {
      const result = await requestService.cancel(request._id, { reason: data.reason });
  ...
  const handleSimpleStatusChange = async () => {
    ...
    try {
      ...
      if (selectedStatus === 'submitted' && request.status === 'draft') {
        // Use submit endpoint for draft -> submitted
        result = await requestService.submit(request._id);
      } else {
        // Use update endpoint for other status changes
        result = await requestService.update(request._id, { status: selectedStatus });
      }
              ...
              <div className="grid grid-cols-2 gap-4">
                <label className="relative">
                  ...
                  <div className={`p-4 border-2 rounded-lg cursor-pointer transition-colors ${
                    watchDecision === 'approved' ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300'
                  }`}>
                ...
                <label className="relative">
                  ...
                  />
                  <div className={`p-4 border-2 rounded-lg cursor-pointer transition-colors ${
                    watchDecision === 'rejected' ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-gray-300'
                  }`}>
                    <div className="flex items-center">
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
              ...
              <button
                ...
                className={`px-6 py-2 text-sm font-medium text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed ${
                  watchDecision === 'approved' ? 'bg-green-600 hover:bg-green-700' :
