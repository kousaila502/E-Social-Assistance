...
const RequestTable: React.FC<RequestTableProps> = ({
  ...
  const getAssignedToName = (request: Demande): string => {
    if (typeof request.assignedTo === 'object' && request.assignedTo) {
      return request.assignedTo.name || 'Unknown User';
    }
    return 'Unassigned';
  };
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {requests.length === 0 ? (
              <tr>
                <td colSpan={userMode ? 3 : 6} className="px-6 py-12 text-center text-gray-500">
