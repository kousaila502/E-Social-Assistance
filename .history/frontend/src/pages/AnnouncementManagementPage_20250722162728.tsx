...
const AnnouncementManagementPage: React.FC = () => {
    ...
    const fetchStats = async () => {
        ...
        try {
            ...
            // Convert announcementsByType array to Record
            const announcementsByType: Record<string, number> = {};
            if (backendStats.announcementsByType && Array.isArray(backendStats.announcementsByType)) {
                backendStats.announcementsByType.forEach((item: { type: string; count: number }) => {
                    announcementsByType[item.type] = item.count;
                });
            }

            setStats({
                totalAnnouncements: backendStats.totalAnnouncements || 0,
                publishedAnnouncements: backendStats.publishedAnnouncements || 0,
                draftAnnouncements: backendStats.draftAnnouncements || 0,
                expiredAnnouncements: backendStats.expiredAnnouncements || 0,
