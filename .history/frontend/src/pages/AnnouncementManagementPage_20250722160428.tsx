...
const AnnouncementManagementPage: React.FC = () => {
    ...
    const fetchStats = async () => {
        ...
        try {
            ...
            // Fix the announcementsByType handling
            let announcementsByType: Record<string, number> = {};

            if (statistics.announcementsByType) {
                if (Array.isArray(statistics.announcementsByType)) {
                    // If it's an array, convert to Record
                    announcementsByType = statistics.announcementsByType.reduce((acc, item) => {
                        acc[item.type] = item.count;
                        return acc;
                    }, {} as Record<string, number>);
                } else if (typeof statistics.announcementsByType === 'object') {
                    // If it's already a Record/object, use it directly
                    announcementsByType = statistics.announcementsByType;
                }
            }
            ...
            setStats({
                ...
                draftAnnouncements: statistics.draftAnnouncements || 0,
                expiredAnnouncements: statistics.expiredAnnouncements || 0,
                totalApplications: statistics.totalApplications || 0,
                pendingApplications: statistics.pendingApplications || 0,
                announcementsByType
            });
        } catch (error: any) {
