...
const RequestManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!canManageRequests) {
      ...
      ErrorHandler.showError('Access denied. You do not have permission to manage requests.');
      return;
    }
  ...
  const handleRequestSelect = async (request: Demande) => {
    try {
      ...
      ErrorHandler.showError('Failed to load request details');
    }
  };
  ...
  const handleStatusUpdate = async (updatedRequest: Demande) => {
    ...
    fetchDashboardStats();
    ErrorHandler.showError('Request status updated successfully!');
  };
  ...
  const refreshData = () => {
    setRefreshTrigger(prev => prev + 1);
    fetchDashboardStats();
    ErrorHandler.showError('Data refreshed');
  };
  ...
  const handleAssignRequest = async (requestId: string, assignedTo: string) => {
    try {
      ...
      setRefreshTrigger(prev => prev + 1);
      fetchDashboardStats();
      ErrorHandler.showError('Request assigned successfully');
    } catch (error: any) {
      console.error('Error assigning request:', error);
      ErrorHandler.showError(error.message || 'Failed to assign request');
    }
  };
  ...
  const handleCancelRequest = async (requestId: string, reason?: string) => {
    try {
      ...
      setRefreshTrigger(prev => prev + 1);
      fetchDashboardStats();
      ErrorHandler.showError('Request cancelled successfully');
    } catch (error: any) {
      console.error('Error cancelling request:', error);
      ErrorHandler.showError(error.message || 'Failed to cancel request');
    }
  };
