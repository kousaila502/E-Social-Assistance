...
const RequestManagementPage: React.FC = () => {
  ...
  const fetchDashboardStats = async () => {
    ...
    try {
      ...
      const transformedStats: QuickStats = {
        ...
        criticalRequests: 0, // Will be calculated from table data or added to backend
  ...
  const handleCancelRequest = async (requestId: string, reason?: string) => {
    try {
      await requestService.cancel(requestId, reason);
      setRefreshTrigger(prev => prev + 1);
      fetchDashboardStats();
  ...
  navigate(ROUTES.SUBMIT_REQUEST); // Uses '/demandes/submit' from constants
};
