...
const AnnouncementManagementPage: React.FC = () => {
    ...
    const fetchStats = async () => {
        ...
        try {
            ...
            const byStatus = backendStats.byStatus || [];
            const byType = backendStats.byType || [];
            const applications = backendStats.applications || [];

            // Convert status array to counts
            const statusCounts = byStatus.reduce((acc, item) => {
                acc[item._id] = item.count;
                return acc;
            }, {});

            // Convert type array to Record
            const announcementsByType = byType.reduce((acc, item) => {
                acc[item._id] = item.count;
                return acc;
            ...
            setStats({
                ...
                publishedAnnouncements: statusCounts.published || 0,
                draftAnnouncements: statusCounts.draft || 0,
                expiredAnnouncements: statusCounts.expired || 0,
