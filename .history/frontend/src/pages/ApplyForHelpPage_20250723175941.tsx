...
// Interface for the create demande response (matching backend structure)
interface CreateDemandeResponse {
  success: true;
  message: string;
  data: {
    demande: {
...
const ApplyForHelpPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await requestService.create(submitData) as any as CreateDemandeResponse;
      ...
      navigate('/my-applications', {
        state: {
          ...
          requestId: response?.data?.demande?._id || 'new-request'
        }
      });
      ...
      if (error?.response?.data?.error?.details) {
        ...
        // Go back to step 2 where most validations happen
        setCurrentStep(2);
      } else {
        // Show general error
        setErrors({
  ...
  const renderStep4 = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Simple error display instead of FormErrorDisplay */}
        {errors.submit && (
