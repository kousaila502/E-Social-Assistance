...
const DashboardPage: React.FC = () => {
  ...
  // FIXED: Use userService.getDashboardStats() instead of getAll()
  ...
  const renderStaffDashboard = () => {
    ...
    // FIXED: Use correct data access for total users
    const totalUsers = totalUsersQuery.data?.statistics?.totalUsers || 0;

    // Debug logs
    console.log('Total Users Query Data:', totalUsersQuery.data);
    console.log('Total Users:', totalUsers);
    console.log('Pending Reviews:', reqStats?.statusBreakdown?.find(s => s.status === 'under_review')?.count ?? 0);
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Quick Actions for Staff - FIXED NAVIGATION */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {hasAnyRole(['admin', 'case_worker']) && (
            <DashboardCard
              ...
              badge={
                (() => {
                  const count = reqStats?.statusBreakdown?.find(s => s.status === 'under_review')?.count ?? 0;
                  console.log('Review Requests button badge count:', count);
        ...
        >
          ...
          <div className="mt-6 flex justify-end">
            <button
              className="inline-flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded hover:bg-indigo-700 transition"
