  ...
  useEffect(() => {
    if (!hasAnyRole(['admin', 'case_worker', 'finance_manager'])) {
      ...
      Promise.all([
        ...
        userService.getAll({ limit: 1 }) // Only need summary, so limit to 1
      ]).then(([reqStats, budgetStats, usersResp]) => {
        setAdminStats({
          totalUsers: usersResp.summary?.totalUsers || usersResp.pagination?.total || 0,
          ...
          approvedToday: 0, // Not available in your API, set to 0 or fetch elsewhere if needed
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {hasAnyRole(['admin', 'case_worker']) && (
          <DashboardCard
            ...
            icon={ClipboardDocumentListIcon}
            link="/demandes" // Use ROUTES.REQUEST_MANAGEMENT or the correct path
        ...
        {hasAnyRole(['admin', 'finance_manager']) && (
          <DashboardCard
            ...
            icon={CurrencyDollarIcon}
            link="/admin/budget-pools" // Use ROUTES.ADMIN.BUDGET or the correct path
      ...
      >
        <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-900">{adminStats.approvedToday}</div>
