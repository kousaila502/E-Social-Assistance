...
const rateLimiter = rateLimit({
  ...
  keyGenerator: (req, res) => {
    ...
    // Let express-rate-limit handle IP properly (including IPv6)
...
const authRateLimiter = rateLimit({
  ...
  onLimitReached: (req, res, options) => {
    console.warn(`Auth rate limit exceeded for ${req.ip} with email ${req.body?.email}`);
  }
});
...
const apiRateLimiter = rateLimit({
  ...
  keyGenerator: (req, res) => {
    ...
    return `api:${rateLimit.defaultKeyGenerator(req, res)}`;
  },
  skip: (req) => {
...
const uploadRateLimiter = rateLimit({
  ...
  keyGenerator: (req, res) => {
    ...
    return `upload:${rateLimit.defaultKeyGenerator(req, res)}`;
  }
});
